...
MeTHists::MeTHists(std::string name, fwlite::TFileService& fs, std::string title) {
    ...
    sumEt_s = dir.make<TH1F>("pt_s", (name+"/pt_s; "+title+" p_T [GeV]; Entries").c_str(),  100,0, 100);
    sumEt_m = dir.make<TH1F>("pt_m", (name+"/pt_m; "+title+" p_T [GeV]; Entries").c_str(),  100,0, 500);
    sumEt_l = dir.make<TH1F>("pt_l", (name+"/pt_l; "+title+" p_T [GeV]; Entries").c_str(),  100,0,1000);
